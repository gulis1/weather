...
pub fn LocationSelector() -> Html {
    ...
    html! {
        <div class="flex flex-col w-screen h-screen items-center p-8 gap-4"
            style={
                "background: linear-gradient(rgba(0, 0, 0, 0.5), rgba(0, 0, 0, 0.6)), url(/assets/bgs/Search.jpg); background-size: cover"
            }
        >

            <div class="flex items-center border-2 border-white border-solid rounded-2xl bg-white text-2xl p-4" >
                <input class="h-full rounded-2xl focus:outline-none" onchange={on_input} ref={input_ref}/>
